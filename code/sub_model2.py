...
class SubjectModel(fluid.dygraph.Layer):
    def __init__(self, max_len, char_size, char_id_len):
        ...
        self.h_conv1d = Conv1d(
            input_dim=256, output_dim=self.char_size, kernel_size=3, activation="relu")
    ...
    def forward(self, inputs: list):
        ...
        # dgcnns = [DilatedGatedConv1d(dim=t_dim, dilation_rate=x)
        #           for x in dilation_rates]
        # for dgcnn in dgcnns:
        #     t = dgcnn(t, mask)

        # pn1 = fluid.layers.fc(
        #     t, self.char_size, num_flatten_dims=2, act="relu")
        ...
        #self_att = att.MultiHead_Attention(8, 16)
...
def position_id(x):
    ...
    batch_size, sent_len = fluid.layers.cast(
        fluid.layers.shape(x), dtype="int64")
        ...
        with fluid.dygraph.guard(place):
            ...
            for epoch in range(EPOCH):
                ...
                data_loader.set_batch_generator(
                    self.data_generate.batch_generator_creator(), places=place)
                for bt_id, data in tqdm(enumerate(data_loader())):
                    ...
                    s1_loss = fluid.layers.cross_entropy(
                        ps1, s1, soft_label=True)
                    s2_loss = fluid.layers.cross_entropy(
                        ps2, s2, soft_label=True)

                    s1_loss = fluid.layers.reduce_sum(
                        s1_loss * mask) / fluid.layers.reduce_sum(mask)

                    s2_loss = fluid.layers.reduce_sum(
                        s2_loss * mask) / fluid.layers.reduce_sum(mask)

                    ave_loss = s1_loss+s2_loss

                    # object_model

                    if bt_id % 100 == 0:
                        print('epoch:{}\tbatch:{}\tloss:{}'.format(
                            epoch, bt_id, ave_loss.numpy()))
                ...
                save_path = os.path.join(
                    os.getcwd(), os.pardir, "dygraph_model")
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                file_name = "%d_sub_model_%.7f" % (
                    epoch, float(ave_loss.numpy()))
